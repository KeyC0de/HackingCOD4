...
#include "../ProcessHacking/os_utils.h"
...
#include "d3d11_vtables.h"
#define VTABLE_PRESENT_OFFSET (UINT)IDXGISwapChainVTable::Present
...
void* pTrampoline = nullptr;		// Pointer to jmp instruction in our trampoline that leads to hookPresent
char ogData[PRESENT_STUB_SIZE];	// Buffer to store original bytes from Present

bool hook( void* pSrc, void* pDst, size_t size );
void writeProtectedMemory( void* pDst, char* pBytes, size_t size );
bool trampolineHook();
bool compileShaderFromString( const char* szShader, const char * szEntrypoint, const char * szTarget, ID3D10Blob ** pBlob );
bool installD3d11Hook( IDXGISwapChain* pSwapchain );
void cleanD3d();
void render();
...
HWND getProcessMainWindow( DWORD dwPID );
BOOL CALLBACK enumWindowsCallback( HWND hWnd, LPARAM lParam );

void threadMain( void* pHandle )
{
	if ( trampolineHook() )
	{
		// END key to unload
	...
	cleanD3d();
	writeProtectedMemory( ogPresent, ogData, PRESENT_STUB_SIZE );
...
BOOL WINAPI DllMain( HINSTANCE hInstanceDll,
	DWORD dwReason,
	LPVOID lpReserved )
{
	switch ( dwReason )
	{
		case DLL_PROCESS_ATTACH:
			...
			CreateThread( nullptr, 0, (LPTHREAD_START_ROUTINE)threadMain, hInstanceDll, 0, nullptr );
			break;
...
bool hook( void* pTargetFun,
	void* pHookedFun,
	size_t nBytes )
{
	DWORD dwOld;
	uintptr_t src = (uintptr_t)pTargetFun;
	uintptr_t dst = (uintptr_t)pHookedFun;

	VirtualProtect( pTargetFun, nBytes, PAGE_EXECUTE_READWRITE, &dwOld );
	HRESULT hres;
	ASSERT_HRES_WIN32_IF_FAILED( hres );

	// relative jump to our hooked Present
	*(char*)src = (char)0xE9;
	*(int*)(src + 1) = (int)(dst - ( src + 5 ) );

	VirtualProtect( pTargetFun,
		nBytes,
		dwOld,
		&dwOld );
	ASSERT_HRES_WIN32_IF_FAILED( hres );
...
{
	DWORD oldProtection;
	!VirtualProtect( p, nBytes, PAGE_EXECUTE_READWRITE, &oldProtection );
...
bool trampolineHook()
{
	...
	ogPresent = (fnPresent)(pVMT[VTABLE_PRESENT_OFFSET]);
	...
	// write jmp to hook
	memcpy( ogData, ogPresent, PRESENT_STUB_SIZE );
	memcpy( pTrampLoc, ogData, PRESENT_STUB_SIZE );
	...
	*(int*)pTrampLoc = (int)(ogPresRet - (uintptr_t)pTrampLoc - 4);
	...
	writeProtectedMemory( pTrampLoc, pJmp, ARRAYSIZE( pJmp ) );
	pTrampLoc = (void*)((uintptr_t)pTrampLoc + ARRAYSIZE( pJmp ));
	*(uintptr_t*)pTrampLoc = (uintptr_t)hookPresent;
#else
...
bool installD3d11Hook( IDXGISwapChain * pSwapchain )
{
	...
	if (!compileShaderFromString( szShadez, "VS", "vs_5_0", &pBlob ))
		return false;
	...
	// create pixel shader
	if (!compileShaderFromString( szShadez, "PS", "ps_5_0", &pBlob ))
		return false;
	...
	if (!numViewports || !pViewports[MAINVP].Width)
	{
		...
		HWND hWnd = getProcessMainWindow( GetCurrentProcessId() );
		RECT rc{ 0 };
